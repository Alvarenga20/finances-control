...
const Transactions = () => {
    ...
    useEffect(() => {
        if (contextTransactions && contextTransactions.length === 0) {
            setTransactions([
                { id: 'T001', amount: 200, category: 'Groceries', date: '2024-12-01', paymentMethod: 'Credit Card', status: 'Completed' },
                { id: 'T002', amount: 500, category: 'Entertainment', date: '2024-11-22', paymentMethod: 'Cash', status: 'Completed' },
                { id: 'T003', amount: 150, category: 'Transport', date: '2024-11-15', paymentMethod: 'Debit Card', status: 'Completed' },
                { id: 'T004', amount: 400, category: 'Dining', date: '2024-10-30', paymentMethod: 'Credit Card', status: 'Pending' },
            ]);
        } else {
    ...
    useEffect(() => {
        const filtered = transactions.filter((transaction) => {
            const withinDateRange = (dateRange.start && dateRange.end)
                ? new Date(transaction.date) >= new Date(dateRange.start) && new Date(transaction.date) <= new Date(dateRange.end)
                : true;
    ...
    const handleSave = (updatedTransaction: Transaction) => {
        setTransactions(prev => prev.map(trans => trans.id === updatedTransaction.id ? updatedTransaction : trans));
    };
            ...
            <div className="p-6 w-full">
                ...
                <div className="mb-6 flex gap-4">
                    ...
                    <input
                        ...
                        className="p-2 border rounded-md w-1/3 bg-white dark:bg-gray-800 dark:text-gray-200 dark:border-gray-700"
                    />
                    <input
                        type="date"
                        value={dateRange.start}
                        onChange={(e) => handleDateRangeChange(e, 'start')}
                        className="p-2 border rounded-md bg-white dark:bg-gray-800 dark:text-gray-200 dark:border-gray-700"
                    />
                    <input
                        type="date"
                        value={dateRange.end}
                        ...
                        <tbody>
                            {filteredTransactions.map((transaction) => (
                                <tr key={transaction.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td className="p-4 border-b">${transaction.amount.toFixed(2)}</td>
